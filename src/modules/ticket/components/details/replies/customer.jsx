...
const TicketReplyCustomer = ({id, description = '', attachments = [], className, ...restProps}) => {
    return (
        <div className="chat__box__text-box flex items-end float-start mb-4">
            <div className="w-10 h-10 hidden sm:flex items-center justify-center relative me-5 text-slate-500 bg-slate-200 rounded-full">
                <Icon type="user" className="h-5 w-5" />
            </div>
            <div className="bg-slate-100 dark:bg-darkmode-400 px-4 py-3 text-slate-500 rounded-e-md rounded-t-md">
                {id} - <PersianNumber>{description}</PersianNumber>
                <div className="mt-1 text-xs text-slate-500"><PersianNumber>2 دقیقه قبل</PersianNumber></div>
