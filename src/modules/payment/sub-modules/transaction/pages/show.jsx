...
import { TransactionList } from "@/modules/payment/sub-modules/transaction/pages/list";
...
export const TransactionShow = () => {
  return (
    <div className="mx-auto grid max-w-2xl grid-cols-1 grid-rows-1 items-start gap-x-4 gap-y-4 lg:mx-0 lg:max-w-none lg:grid-cols-3">
      {/* Invoice summary */}
          ...
          <dl className="flex flex-wrap">
            ...
            <div className="w-full border-t mt-6 pt-3 pb-5">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 text-end">
                  {invoice.from ? (
                    <time dateTime={invoice.from}>
                      <PersianNumber>{invoice.fromPersian}</PersianNumber>
                    </time>
                  ) : null}
                </dd>
              </div>
              <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:py-3 sm:px-6">
                ...
                <dd className="mt-1 text-sm text-gray-900 sm:col-span-2 sm:mt-0 text-end">
                  {invoice.to ? (
                    <time dateTime={invoice.to}>
                      <PersianNumber>{invoice.toPersian}</PersianNumber>
                    </time>
                  ) : (
                    "-"
                  )}
        ...
        <table className="w-full whitespace-nowrap text-start text-sm leading-6">
          ...
          <thead className="border-b border-gray-200 text-gray-900">
            <tr>
              ...
              <th
                scope="col"
                className="hidden py-3 ps-8 pe-0 text-end font-semibold sm:table-cell"
              >
                میزان مصرف (ساعت)
              </th>
              <th
